...
if (isset($_POST['bayar'])) {
    ...
    if ($id != null) {
        ...
        $barang = $conn->query("SELECT a.*, b.* , SUM(a.jml_barang) as total 
        ...
        if ($trx > 0) {
            $query = $conn->query("UPDATE transaksi SET status = 'bayar' WHERE id_trx = '$id'");
            if ($query) {
            ...
            $query = $conn->query("UPDATE transaksi SET status = 'cannot_proceed' WHERE id_trx = '$id'");
            if ($query) {
